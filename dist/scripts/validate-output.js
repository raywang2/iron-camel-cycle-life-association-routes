...
const actualGpxPaths = new Set(gpxFiles.filter((entry) => entry.isFile() && entry.name.endsWith(".gpx")).map((entry) => `gpx/${entry.name}`));
...
for (const day of routes) {
    ...
    if (day.type === "ride") {
        ...
        assert(typeof day.generatedDistanceKm === "number" && day.generatedDistanceKm > 0, `Day ${day.day} needs a positive generated distance`);
        assert(typeof day.distanceDeltaKm === "number", `Day ${day.day} needs a numeric distance delta`);
        ...
        assert(day.geojson.coordinates.length > day.waypoints.length, `Day ${day.day} geometry must have more points than waypoints`);
        assert(day.geojson.coordinates.length >= 20, `Day ${day.day} geometry must contain at least 20 points`);
...
assert(actualGpxPaths.size === expectedGpxPaths.size &&
    [...actualGpxPaths].every((path) => expectedGpxPaths.has(path)) &&
    [...expectedGpxPaths].every((path) => actualGpxPaths.has(path)), `GPX files must exactly match riding days`);
